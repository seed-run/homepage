...
2. Or configure it using the `seed.yml`
   ...
   Note that this build hook will be run in your project root. So if you want to run it for a specific service you'll need something like this: 
 
   ``` yml
   before_build:
     - cd my-service/ && serverless create_domain --stage $SEED_STAGE_NAME
   ```
